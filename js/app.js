document.addEventListener("DOMContentLoaded", function() {
  ...
  function stopTimer() {
    ...
    let circPos   = getComputedStyle(countDown.circSVG).getPropertyValue('stroke-dashoffset'),
  ...
  function countDownDisplay(time) {
    ...
    countInt = setInterval(() => {
      ...
      if(time < 0) {
				let audio = new Audio('https://notificationsounds.com/soundfiles/352fe25daf686bdb4edca223c921acea/file-sounds-805-happy-jump.wav');
        clearInterval(countInt);
        audio.play();
        // Reset timer animation
  ...
  function timeSetters(i, arr) {
    ...
    arr[1][i].addEventListener('click', function() {
      if(arr[0][i] < 60 && timer.running === false) {
        ...
        if(this.id === 'study-up') { countDown.counter.textContent = `${arr[0][0]}:00`; };
      }
    });
    ...
    arr[2][i].addEventListener('click', function() {
      if(arr[0][i] > 1 && timer.running === false) {
        ...
        arr[3][i].textContent = arr[0][i];
        arr[0][0] = arr[0][0] < 10 ? `0${arr[0][0]}` : arr[0][0];
        if(this.id === 'study-down') { countDown.counter.textContent = `${arr[0][0]}:00`; };
      }
    });
  }
  ...
  countDown.timerSVG.addEventListener('click', () => {
    startStop();
  });
